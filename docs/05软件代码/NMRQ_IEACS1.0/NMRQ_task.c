...
OS_STK        TASK_ParseStk[TASK_STK_SIZE];
...
void  TASK_Start(void *pdata)
{
    ...
    //OSTaskCreate(TASK_Parse, (void *)0, &TASK_ParseStk[TASK_STK_SIZE - 1], TASK_PRIO_PARSE);
    ...
    {
		/*	系统级的操作	*/	
		//COM_SendDataDown("StartTask starts to run\n");
...
void  TASK_KeyDec(void *pdata)
{
    ...
    {	/*	判断是否有按键按下	*/
		...
		{													/*	得到键值及按键类型	*/
		   ...
		   OSTaskResume(TASK_PRIO_PARSE);					/*	唤醒TASK_Parse任务进行按键信息解析	*/
...
void  TASK_PcDec(void *pdata)
{
    ...
    for (;;)																	
    {
		...
		COM_SendDataDown("PcDec starts to run\n"); 
		if((U0LSR & 0x01)==0);						  //若无数据则不执行
    ...
    for (;;)																	
    {
		if(OSIsKeyDown != KEY_UNCLICKED)  					/*	查看是否是按键信息	*/
		{
			...
			{	/*	当前界面为主界面	*/
				...
				{	/*	当前按键类型为长按	*/
				 	if(uiTempKeyValue == KEY_SWI3)				/*	长按KEY_SWI3的操作 进入设置选择界面	*/
					{
						LCDCurrentPage = LCDCurrentPage->lcdPageNextPage[0];
						LCDCurrentFocus = 0;
						//LCD显示刷新/////////////////////////////////////////////////////////////////////
					}
					if(uiTempKeyValue == KEY_SWI4)				/*	长按KEY_SWI4的操作 进入电器选择界面	*/
					{
				...
				{	/*	当前按键类型为短按	*/
				 	if(uiTempKeyValue == KEY_SWI1)			/*	短按KEY_SWI1的操作 对门灯进行开关操作	*/
					{
						//门灯的开关操作/////////////////////////////////////////////////////////////////////
					...
					if(uiTempKeyValue == KEY_SWI2)			/*	短按KEY_SWI2的操作 对卧室灯进行开关操作	*/
					{
						//卧室灯的开关操作/////////////////////////////////////////////////////////////////////
					...
					if(uiTempKeyValue == KEY_SWI3)			/*	短按KEY_SWI3的操作 对空调进行开关操作	*/
					{
						//空调的开关操作/////////////////////////////////////////////////////////////////////
					...
					if(uiTempKeyValue == KEY_SWI4)			/*	短按KEY_SWI4的操作 对热水器进行开关操作	*/
					{
						//热水器的开关操作/////////////////////////////////////////////////////////////////////
			...
			{	/*	当前LCD界面为非主界面	*/
				if(uiTempKeyType == KEY_SHORTCLICKED)			
				{	/*	当前按键类型为短按	*/
				 	if(uiTempKeyValue == KEY_SWI2)			/*	短按KEY_SWI2的操作 确认操作	*/
					{
						//门灯的开关操作/////////////////////////////////////////////////////////////////////
						uiTempOpeState = HI_Control(OS_STATE_INFO_TYPE_SWITCH,OS_STATE_CTRL_OBJ_DOOR_LIGHT,!(OSStateInfo[0].uiOSStateContent));
						//如果操作成功 LCD显示刷新/////////////////////////////////////////////////////////////////////
					...
					if(uiTempKeyValue == KEY_SWI4)			/*	短按KEY_SWI4的操作 返回上一级操作	*/
					{
						//卧室灯的开关操作/////////////////////////////////////////////////////////////////////
						uiTempOpeState = HI_Control(OS_STATE_INFO_TYPE_SWITCH,OS_STATE_CTRL_OBJ_BEDROOM_LIGHT,!(OSStateInfo[1].uiOSStateContent));
						//如果操作成功LCD显示刷新/////////////////////////////////////////////////////////////////////
					...
					if(uiTempKeyValue == KEY_SWI1)			/*	短按KEY_SWI1的操作 加 1 操作	*/
					{
						//空调的开关操作/////////////////////////////////////////////////////////////////////
						uiTempOpeState = HI_Control(OS_STATE_INFO_TYPE_SWITCH,OS_STATE_CTRL_OBJ_AIRCONDITION,!(OSStateInfo[2].uiOSStateContent));
						//如果操作成功LCD显示刷新/////////////////////////////////////////////////////////////////////
					...
					if(uiTempKeyValue == KEY_SWI3)			/*	短按KEY_SWI3的操作 减 1 操作	*/
					{
						//热水器的开关操作/////////////////////////////////////////////////////////////////////
						uiTempOpeState = HI_Control(OS_STATE_INFO_TYPE_SWITCH,OS_STATE_CTRL_OBJ_CALORIFIER,!(OSStateInfo[3].uiOSStateContent));
						//如果操作成功LCD显示刷新/////////////////////////////////////////////////////////////////////
...
{
	INT8U cInstruction[9][13];    //保存翻译数据结果的数组
	INT8U  i;					  //循环计数变量
...
//	for(i=0;i<9;i++)
//	{	
//		INT8U * tmpStr;				  //临时保存待发送指令
//		tmpStr=cInstruction[i];
